$(function(){
	...
	//related thesis through submitting keywords
	function getRelated (event)
	{	
		//return search to null value
		$('#rel_th_ul').empty();
		//val comparison on thesis for no redundant
		...
		{	
			if (response.status == 'OK')
			{	
				//if none related thesis
				if ($.isEmptyObject(response.rel))
				{
					$('#rel_th_ul').prepend("\"Sorry! Unable to find related thesis.\"");
				}
